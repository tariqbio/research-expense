...
export default function ProjectDetail() {
  ...
  const handleExportXLSX = () => {
    ...
    // Sheet 1: Summary
    const summaryData = [
      ["RESEARCH EXPENSE TRACKER - PROJECT REPORT"],
      ...
      ["Total Budget", budget],
      ["Total Spent", spent],
      ["Reimbursed", reimbursedExp.reduce((a, e) => a + Number(e.amount), 0)],
      ["Pending", pendingExp.reduce((a, e) => a + Number(e.amount), 0)],
      ["Remaining", budget - spent],
      ["Utilised (%)", ((spent / budget) * 100).toFixed(2)],
      [],
      ["FUND INSTALLMENTS"],
      ["Total Allocated", totalInstalled],
      ["Received", receivedFunds],
      ["Pending", totalInstalled - receivedFunds],
    ];
    ...
    const expenseData = [
      ...
      [],
      ["Date", "Researcher", "Category", "Description", "Amount", "Status"],
      ...expenses.map(e => [
      ...
      [],
      ["Total", expenses.length + " records", "", "", expenses.reduce((a, e) => a + Number(e.amount), 0), ""],
    ];
    const expenseSheet = XLSX.utils.aoa_to_sheet(expenseData);
    expenseSheet["!cols"] = [{wch: 12}, {wch: 15}, {wch: 15}, {wch: 25}, {wch: 15}, {wch: 12}];
    XLSX.utils.book_append_sheet(wb, expenseSheet, "Expenses");
    
    // Sheet 3: Fund Installments
    const instData = [
      ["FUND INSTALLMENTS", project.installments.length + " installment(s)"],
      [],
      ["#", "Expected Date", "Amount", "Status", "Date Received", "Note"],
      ...project.installments.map((inst, idx) => [
        "#" + (idx + 1),
        inst.expected_date ? fmtDate(inst.expected_date) : "—",
        Number(inst.amount),
        inst.status,
    ...
    instSheet["!cols"] = [{wch: 5}, {wch: 15}, {wch: 15}, {wch: 12}, {wch: 15}, {wch: 20}];
    XLSX.utils.book_append_sheet(wb, instSheet, "Installments");
    
    // Sheet 4: Member Summary
    const memberData = [
      ["MEMBER SUMMARY"],
      [],
      ["Name", "Email", "Role", "Expenses", "Total", "Reimbursed", "Pending"],
      ...project.members.map(m => [
        m.name,
        m.email,
        m.role,
        expenses.filter(e => e.submitted_by_id === m.id).length,
        expenses.filter(e => e.submitted_by_id === m.id).reduce((a, e) => a + Number(e.amount), 0),
        expenses.filter(e => e.submitted_by_id === m.id && e.reimbursed).reduce((a, e) => a + Number(e.amount), 0),
        expenses.filter(e => e.submitted_by_id === m.id && !e.reimbursed).reduce((a, e) => a + Number(e.amount), 0),
      ]),
    ];
    const memberSheet = XLSX.utils.aoa_to_sheet(memberData);
    memberSheet["!cols"] = [{wch: 15}, {wch: 20}, {wch: 12}, {wch: 10}, {wch: 15}, {wch: 15}, {wch: 15}];
    XLSX.utils.book_append_sheet(wb, memberSheet, "Members");
    
    XLSX.writeFile(wb, `${project.code}-report-${new Date().toISOString().split("T")[0]}.xlsx`);
  };
