...
router.post('/request-access', async (req, res) => {
  const { name, email, password, workspace_name, report_header, position, message } = req.body;
    ...
    await pool.query(
      `INSERT INTO pending_signups
       ...
       ON CONFLICT (email) DO UPDATE SET
         ...
         report_header=$5, position=$6, message=$7, created_at=NOW()`,
...
router.get('/pending-signups', authenticate, superOnly, async (req, res) => {
  try {
    const { rows } = await pool.query(
      'SELECT id,name,email,workspace_name,report_header,position,message,created_at FROM pending_signups ORDER BY created_at DESC'
    );
    res.json(rows);
